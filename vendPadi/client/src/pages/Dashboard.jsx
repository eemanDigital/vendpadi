...
const Dashboard = () => {
  ...
  const handleAddNew = () => {
    if (isAtLimit) {
      ...
      toast.error(
        `Your ${displayPlan} plan allows only ${currentLimit} products. Upgrade to add more.`,
      );
      return;
    }
    ...
    <div className="min-h-screen bg-gray-50">
      <aside className="hidden lg:block fixed left-0 top-0 w-64 h-screen bg-navy text-white p-6 flex flex-col overflow-y-auto z-20">
        ...
        <div className="mb-4">
          ...
          <h3 className="font-sora font-semibold text-center truncate px-2">
            {vendor?.businessName}
          </h3>
          ...
          <div className="mt-4 bg-white/10 rounded-xl p-3">
            ...
            <div className="w-full bg-white/20 rounded-full h-1.5">
              <div
                ...
                style={{
                  width: `${currentLimit === Infinity ? 0 : Math.min((products.length / currentLimit) * 100, 100)}%`,
                }}
              />
            </div>
          </div>
        ...
        {lowStockProducts.length > 0 && (
          <LowStockAlert
            ...
            onManageClick={() => setShowInventoryModal(true)}
          />
        )}

        {hasAnalytics && <GrowthInsights analytics={analytics} />}
        ...
        {!isOnTrial && planType !== "premium" && (
          <div className="mt-4 p-4 bg-gold/20 rounded-xl border border-gold/30">
            ...
            <p className="text-xs text-gray-400 mb-3">
              Unlock more features & products
            </p>
            <button
              onClick={() => setShowUpgradeModal(true)}
              className="text-xs bg-gold hover:bg-gold/90 text-white px-3 py-1.5 rounded-lg font-medium">
              Upgrade Now
            </button>
          </div>
        )}
        ...
        <div className="max-w-7xl mx-auto p-3 sm:p-4 lg:p-6">
          {hasAnalytics && (
            <div className="mb-4 sm:mb-6">
              <div
                className={`grid gap-2 sm:gap-3 mb-3 sm:mb-4 ${hasFullAnalytics ? "grid-cols-2 lg:grid-cols-5" : "grid-cols-2 lg:grid-cols-4"}`}>
                <AnalyticsCard
                  title="Store Views"
                ...
                <AnalyticsCard
                  ...
                  value={
                    analyticsLoading ? "..." : analytics?.whatsappClicks || 0
                  }
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
            <div>
              <h1 className="font-sora font-bold text-2xl text-navy">
                Products
              </h1>
              <p className="text-gray-500 text-sm">
                {filteredProducts.length} of {products.length} products
                {effectivePlan !== "free" &&
                  ` • ${planLimits.images} images max`}
              </p>
            </div>
            <div className="flex items-center gap-2">
              <a
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-600">
                    Your {isOnTrial ? "Premium (Trial)" : vendor?.plan?.type}{" "}
                    plan allows {currentLimit} products.
                  </p>
                </div>
              </div>
              <Link
                to="/settings"
                className="bg-gold hover:bg-gold/90 text-white px-5 py-2 rounded-xl font-medium transition-colors whitespace-nowrap">
                Upgrade Plan
              </Link>
            </div>
          )}
          ...
          {loading ? (
            ...
            <motion.div layout className={`grid ${gridCols} ${gridGap}`}>
