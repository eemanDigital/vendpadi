          ...
          {Object.entries(PLAN_DETAILS).map(([planKey, plan]) => {
            ...
            return (
              <motion.div
                ...
                whileHover={{ y: -8, boxShadow: isBusiness ? "0 30px 60px rgba(16, 185, 129, 0.3)" : "0 20px 40px rgba(0, 0, 0, 0.15)" }}
              ...
              >
                {plan.popular && !isBusiness && (
                  <motion.div 
                    className="absolute -top-4 left-1/2 -translate-x-1/2 px-5 py-1.5 rounded-full text-sm font-bold whitespace-nowrap shadow-lg z-10 bg-gradient-to-r from-padi-green to-padi-green-dark text-white"
                    initial={{ scale: 0 }}
                    whileInView={{ scale: 1 }}
                    viewport={{ once: true }}
                  >
                ...
                {planKey === "premium" && (
                  <motion.div 
                    className="absolute -top-4 left-1/2 -translate-x-1/2 px-5 py-1.5 rounded-full text-sm font-bold whitespace-nowrap shadow-lg z-10 bg-gradient-to-r from-amber-500 to-orange-500 text-white"
                    initial={{ scale: 0 }}
                    whileInView={{ scale: 1 }}
                    viewport={{ once: true }}
                  >
                ...
                {billingCycle === "yearly" && plan.price > 0 && planKey !== "premium" && (
                  <motion.div 
                    className="absolute -top-4 right-2 px-3 py-1 rounded-full text-xs font-bold whitespace-nowrap shadow-lg z-10 bg-gradient-to-r from-green-500 to-emerald-500 text-white"
                    initial={{ scale: 0 }}
                    whileInView={{ scale: 1 }}
                    viewport={{ once: true }}
                  >
        ...
        <ScrollReveal delay={400}>
          <div className="mt-12 space-y-4">
            <div className="text-center">
              <motion.div 
                className="inline-flex items-center gap-3 bg-gradient-to-r from-amber-50 to-orange-50 border border-amber-200 text-amber-800 px-6 py-4 rounded-2xl text-sm font-medium"
                whileHover={{ scale: 1.02 }}
              >
                <motion.div
                  animate={{ rotate: [0, 10, -10, 0] }}
            ...
            <div className="text-center">
              <motion.div 
                className="inline-flex items-center gap-3 bg-gradient-to-r from-green-50 to-emerald-50 border border-green-200 text-green-700 px-6 py-4 rounded-2xl text-sm font-medium"
                whileHover={{ scale: 1.02 }}
              >
