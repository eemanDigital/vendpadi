...
import { FiX, FiPlus, FiTrash2, FiGift, FiZap, FiCheck, FiLock } from 'react-icons/fi';
...
const BundleManager = ({ products, onUpgradeClick }) => {
  ...
  const handleToggleDeal = async (bundle) => {
    try {
      ...
      setBundles(prev => prev.map(b => b._id === bundle._id ? data.bundle : b));
      toast.success(bundle.isDealOfTheDay ? 'Deal removed' : 'Now a Deal of the Day!');
    } catch (error) {
      ...
      <div className="bg-white rounded-2xl border border-gray-100 p-5">
        ...
        <div className="bg-gray-50 rounded-xl p-4 flex items-center gap-3">
          ...
          <p className="text-sm text-gray-500 flex-1">
            Upgrade to Premium to create product bundles and deals
          </p>
          <button
            onClick={onUpgradeClick}
            className="text-padi-green text-sm font-medium hover:underline whitespace-nowrap"
          >
            Upgrade
          </button>
        </div>
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await onSubmit({
        ...
        isDealOfTheDay,
        dealEndTime: isDealOfTheDay ? new Date(Date.now() + 24 * 60 * 60 * 1000) : null
      });
    } finally {
      setSubmitting(false);
    }
    ...
    >
      ...
      >
        <div className="flex items-center justify-between p-5 border-b border-gray-100">
          <h3 className="font-sora font-bold text-lg text-navy">Create Bundle</h3>
        ...
        <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-5 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="flex items-end">
              <div className={`flex-1 p-3 rounded-xl text-center ${discountPct > 0 ? 'bg-green-50' : 'bg-gray-50'}`}>
                <p className={`text-lg font-bold ${discountPct > 0 ? 'text-green-600' : 'text-gray-400'}`}>
                  {discountPct > 0 ? `${discountPct}% OFF` : 'No discount'}
                </p>
                {savings > 0 && (
                  <p className="text-xs text-gray-500">Save ₦{savings.toLocaleString()}</p>
                )}
              </div>
            </div>
          </div>
          ...
          <label className="flex items-center gap-3 p-3 bg-red-50 rounded-xl cursor-pointer">
            ...
            <div className="flex-1">
              <p className="font-medium text-sm text-red-800">Deal of the Day</p>
              <p className="text-xs text-red-600">Featured prominently on your store</p>
            </div>
